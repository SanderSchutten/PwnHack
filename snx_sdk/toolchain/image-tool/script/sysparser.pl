...
#uexce related
my $uexce_command ="$tool_dir/image_tool";
my $header_file = "$header_dir/no-os/header.bin"; 
my $uboot_file = "$obj_dir/uexce.bin.d";
my $u_boot_env_file = "$obj_dir/u-boot-env.bin.d";
my $flash_layout_header = "$obj_dir/flash_layout.h";
my $flash_layout_header_name = "flash_layout.h";
my $flash_layout_file = "$obj_dir/flash_layout.bin.d";

my $hw_setting_image = "$obj_dir/hw_setting.image.d";
...
my $u_boot_image_d = "$obj_dir/u_boot.image.d";
my $u_boot_env_bin_d = "$obj_dir/u-boot-env.bin.d";
...
#crc
my $crc_file = "$obj_dir/crc.bin";
my @crc_value = 0;

my $spi_file_d = "$obj_dir/spi_flash_layout.d";       
my $spi_type_d = "SF";
my $nand_file_d = "$obj_dir/nand_flash_layout.d";     
my $nand_type_d = "NAND";
...
#system("rm $header_dir/linux/firmware_update");
#system("rm $header_dir/no-os/$flash_layout_header_name");
...
sub generate_selfburn_firmware
{
	...
	my $image_file = "$obj_dir/image.d";
	my $config_file = "$obj_dir/config.d";
	...
	if ($target_name =~ /firmware_f/i) {
		...
		if ($st58660_type eq "y") {
			my $muti_image = "$obj_dir/muti.image.d";
			my @config_pad;
			my $config_size;
			my $config_pad_size;
			...
			my $config_crc16 = 0;
		 	my $config_tmp_file = "$obj_dir/config_tmp.d";
		 	my $config_tmp;
		 	
		 	open(oFILE, $config_file) or die "can't open '$config_file': $!";
		 	binmode(oFILE);
			read(oFILE, $config_tmp, $config_size);
	  		close oFILE;
	  		
  	  		open oFILE, ">" . $config_tmp_file or die "Can't creat '$config_tmp_file': $!";
	 		print oFILE $config_tmp;
			close oFILE;
	
		 	system("$tool_dir/crc_calculation $config_tmp_file $crc_file");
		 	open(GIF, $crc_file) or die "can't open '$crc_file': $!";
		 	@crc_value = <GIF>;
			close GIF;
			$config_crc16 = $crc_value[0];
...
#header.bin
	system("make -C $header_dir/no-os/ clean;make -C $header_dir/no-os/ FW_BURN_FLOW=yes");
...
#IMAGE_TABLE
	...
	if($target_name eq $FIRMWARE_NAME){
...
#############	CRC16 IMAGE ##########
	...
	my $image_size;
	my $image_pad_size;
	my $firmware_f_md5_file = "$obj_dir/firmware_md5_file.bin";
	...
	close oFILE;
		...
		my $md5_encrypt_file = "$obj_dir/md5_encrypt.bin";
		system("$tool_dir/md5_encrypt $file_md5 $md5_encrypt_file"); 
	...
	my $image_crc16 = 0;
	if ($target_name =~ /firmware_f/i) {
	 	system("$tool_dir/crc_calculation $image_file $crc_file");
	 	open(GIF, $crc_file) or die "can't open '$crc_file': $!";
	 	@crc_value = <GIF>;
		close GIF;
		$image_crc16 = $crc_value[0];
...
#IMAGE
	...
	if ($target_name eq "firmware") {
		system("make -C $header_dir/linux/ clean;cp $target_file $header_dir/linux/FIRMWARE.bin;make -C $header_dir/linux/");
		system("cp -f $header_dir/linux/firmware_update $target_file");
...
{		
	...
	my $pz_ufile = "$obj_dir/uboot.d";
	my $config = $_[8];

############################# Config processing  ################################
	my $hw_setting_af_ddr = "$config_path/hw-setting/template/hw_setting_af_ddr_asic.txt";
	if ($st58660_type eq "y") {
		$hw_setting_af_ddr = "$config_path/hw-setting/template/hw_setting_af_ddr_98660asic.txt";
	}
	...
	my $hw_template_file = "$config_path/hw-setting/template/hw_setting_af_pll_asic.txt";
	if ($st58660_type eq "y") {
		$hw_template_file = "$config_path/hw-setting/template/hw_setting_af_pll_98660asic.txt";
	}
...
#	}
	...
	elsif (($platform =~ /sn98606/)){
  		my $hw_which_file = "sn98606";
  		unless ($hw_version =~ /none/){
  			$hw_which_file = $hw_which_file."_".$hw_version;
  		}
			$ddr_config_file = $ddr_config_file.$hw_which_file."/";
 ...
 #       system ("touch $ddr_config_58660_end");
	...
	if ($ddr_freq =~ /25/){
		if ($st58660_type =~ /^y$/) {
			...
			$hw_template_file = "$config_path/hw-setting/template/hw_setting_af_pll_fpga.txt";
			$hw_setting_af_ddr = "$config_path/hw-setting/template/hw_setting_af_ddr_fpga.txt";
			$project = $ddr_project_para;
...
#		system ("make -C $config_path/hw-setting/ sdk-f WHICH_VALUE=$ddr_config_58660_end PLATFORM=$witch_platform");
	...
	############### ADD ##########################
	$uexce_command = $uexce_command." -e $header_file";
	...
	my $muti_main_fir = "$obj_dir/hw_setting.image.main.d";
	my $muti_f00_fir = "$obj_dir/hw_setting.image.f00.d";
	my $muti_f01_fir = "$obj_dir/hw_setting.image.f01.d";
	my $muti_f02_fir = "$obj_dir/hw_setting.image.f02.d";
	my $muti_end_fir = "$obj_dir/hw_setting.image.end.d";
	my $hw_setting_file = $_[9];

	if ($st58660_type =~ /^y$/) {
		# main
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.main.txt.d";
		foreach (@data_hw){
			unless ($_ =~ /^\#/){
				print oHW_SETTING;
			}
		...
		close oHW_SETTING;
		system("touch $header_file");
		system("$tool_dir/image_tool -h $obj_dir/hw_setting.main.txt.d -o $obj_dir/ -e $header_file");
		my $file_len = -s $hw_setting_file;
		system("mv $hw_setting_file $muti_main_fir");

		#end main
		#df00 
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.f00.txt.d";
		...
		# -e just for compile
		system("$tool_dir/image_tool -h $obj_dir/hw_setting.f00.txt.d -o $obj_dir/ -e $header_file");
		system("mv $hw_setting_file $muti_f00_fir");
		#end df00
		#df01 
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.f01.txt.d";
		...
		close oHW_SETTING;
		system("$tool_dir/image_tool -h $obj_dir/hw_setting.f01.txt.d -o $obj_dir/ -e $header_file");
		system("mv $hw_setting_file $muti_f01_fir");
		#end df00
		#df02
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.f02.txt.d";
		...
		close oHW_SETTING;
		system("$tool_dir/image_tool -h $obj_dir/hw_setting.f02.txt.d -o $obj_dir/ -e $header_dir");
		system("mv $hw_setting_file $muti_f02_fir");
		#end df00
		#end ddr
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.end.txt.d";
		...
		close oHW_SETTING;
		system("$tool_dir/image_tool -h $obj_dir/hw_setting.end.txt.d -o $obj_dir/ -e $header_file");
		system("mv $hw_setting_file $muti_end_fir");
	} else {	
		my $last_line = "";
		my $fill = "empty";
		my $find_030 = "empty";
		open oHW_SETTING, ">" . "$obj_dir/hw_setting.txt.d";
		foreach (@data_hw){
			if(($_ =~ /^\#/)||($_ =~ /^b/)){
				print oHW_SETTING;
			}
		...
		$uexce_command = $uexce_command." -h $obj_dir/hw_setting.txt.d";
	}
	...
	if ($st58660_type =~ /^y$/) {
		...
		my $hw_setting_crc16 = 0;
	 	system("$tool_dir/crc_calculation $hw_setting_file $crc_file");
	 	open(GIF, $crc_file) or die "can't open '$crc_file': $!";
	 	@crc_value = <GIF>;
		close GIF;
		$hw_setting_crc16 = $crc_value[0];
	...
	$env_file = $config_path."/u-boot-env/".$env_file;
	...
	system("mv $flash_layout_header $header_dir/no-os/");
	system("make -C $header_dir/no-os/ clean;make -C $header_dir/no-os/ FW_BURN_FLOW=no");
	$uexce_command = $uexce_command." -l ".$flash_layout_file;

	$uexce_command = $uexce_command." -v 0x12345678";
	$uexce_command = $uexce_command." -o $obj_dir/";
	...
	foreach(@env_data){
		...
		if($record_start =~ /^yes$/i){
			...
			if ($env_arg =~ /^bootargs=$/i) {
				...
				$tmp_value = $flash_size * 1024 - $used;
				if ($tmp_value > 0){
					$env_cmd = $env_cmd.",".$tmp_value."k(unused)";
				}
...
sub padd_env
{
  ...
  unless ($what_large == 0){
	...
	my $crc16;
 	...
 	system("$tool_dir/crc_calculation $file $crc_file");
 	open(GIF, $crc_file) or die "can't open '$crc_file': $!";
 	@crc_value = <GIF>;
	close GIF;
	$crc16 = $crc_value[0];
...
sub fun_flash_layout_bin
{
	...
	### Attach CRC16 [16-bytes] ###
	my $crc16;
 	system("$tool_dir/crc_calculation $$filelayout_file $crc_file");
 	open(GIF, $crc_file) or die "can't open '$crc_file': $!";
 	@crc_value = <GIF>;
	close GIF;
	$crc16 = $crc_value[0];
