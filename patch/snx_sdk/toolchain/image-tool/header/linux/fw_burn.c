...
int snx_flash_erase(char *dev_name, unsigned int start_offset, unsigned int end_offset)
{
	int fd;
	int i, count;
	struct mtd_info_user mtd;
	...
	erase.length = end_offset - start_offset; 
	erase.start = start_offset;
	...
	while(pmtd_info != NULL)
	{
		...
		)
		{
			if(erase_flag == 0){
				...
				ret = snx_flash_erase(pmtd_info->name, start_addr - pmtd_info->start_addr,
						end_addr - pmtd_info->start_addr);
			}
			break;
		}
		...
		)
		{
			if(erase_flag == 0){
				...
				ret = snx_flash_erase(pmtd_info->name, start_addr - pmtd_info->start_addr,
						pmtd_info->end_addr - pmtd_info->start_addr);
			}
			start_addr = pmtd_info->end_addr;
		}
